...
def parse_args():
    ...
    parser.add_argument(
        '--test_img_size', type=int, default=832, help="test image size")
    parser.add_argument(
...
if __name__ == '__main__':
    ...
    config.DATASET.MGDPT_IMG_RESIZE = args.test_img_size
    ...
    if args.refine:
        model = PLCascadeRefineMatcher(config, pretrained_ckpt=args.ckpt_path, profiler=profiler, dump_dir=args.dump_dir)
    else:
        model = PLCascadeMatcher(config, pretrained_ckpt=args.ckpt_path, profiler=profiler, dump_dir=args.dump_dir)
    ...
    # lightning trainer
    trainer = pl.Trainer.from_argparse_args(args, replace_sampler_ddp=False, logger=False, precision=16)
