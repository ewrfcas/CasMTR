...
batch_size=1
pin_memory=true
exp_name="CasMTR-4c-stage${TRAINING_STAGE}-size${TRAIN_IMG_SIZE}-bs$(($n_gpus_per_node * $n_nodes * $batch_size))"
...
python -u ./train.py \
  ...
  --precision=16 \
  --amp_level='O1' \
  --resume_from_checkpoint='logs/tb_logs/CascadeMatcher-stage1-704-bs8-V0/version_0/check_points/last_for_stage4.ckpt' \
  --reset_lr