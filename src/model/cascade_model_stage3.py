...
class CasMTR(nn.Module):
    def __init__(self, config):
        ...
        self.cascade_matching_4c = CascadeMatching(get_match_config(config['match_cascade'], 0), config['coarse2'])

        self.fine_preprocess = CascadeFinePreprocess(config, config['fine'], config['coarse2'], coarse_level='4c')
        self.loftr_fine = LocalFeatureTransformer(config["fine"], train_size // 2)
        self.fine_matching = CascadeFineMatching(coarse_level='4c')
    ...
    def forward(self, data):
        ...
        if self.stage >= 2:
            ...
            feat_8c1 = rearrange(feat_8c1, 'b (h w) c -> b c h w', h=data['hw0_8c'][0], w=data['hw0_8c'][1])
            ...
            # sort the 1d-index in windows, ensure the cross softmax can be optimized correctly
            # idx_4c01 = torch.sort(idx_4c01, dim=2, descending=False)[0]  # [B,HW,4ww]
            # idx_4c10 = torch.sort(idx_4c10, dim=2, descending=False)[0]
            ...
            feat_f0_unfold, feat_f1_unfold = self.fine_preprocess.forward(feat_f0, feat_f1, feat_4c0, feat_4c1, data)
