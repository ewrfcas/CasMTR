...
from src.optimizers import build_scheduler
...
def torch_init_model(model, total_dict, key, prefix='', rank=0):
    if key in total_dict:
        state_dict_ = total_dict[key]
    else:
        state_dict_ = total_dict
    missing_keys = []
    unexpected_keys = []
    error_msgs = []
    state_dict = {}
    for k in state_dict_:
        state_dict[k.replace(prefix, '')] = state_dict_[k]
...
def build_optimizer(model, config):
    ...
    frozen_list = ['backbone.', 'loftr_coarse.']

    param_groups = []
    for n, p in model.named_parameters():
        if any([f in n for f in frozen_list]):
            continue
        if p.requires_grad:
            is_vit = '.vit.' in n
...
class PLCascadeRefineMatcher(pl.LightningModule):
    def __init__(self, config, pretrained_ckpt=None, profiler=None, dump_dir=None, reset_lr=False):
        ...
        self.loss = CascadeLoss(_config, opt_coarse=False)
        self.loss_dict = collections.defaultdict(float)
        self.first_train = True

        if os.path.exists(_config['loftr']['quadtree_path']):
            quadtree_weights = torch.load(_config['loftr']['quadtree_path'], map_location='cpu')
            torch_init_model(self.matcher, quadtree_weights, 'state_dict', prefix='matcher.', rank=0)
            if self.global_rank == 0:
                print('Loading QuadTree success.')
        else:
            if self.global_rank == 0:
                print('WARNING: No QuadTree weight is found, if testing you can ignore it.')
    ...
    def optimizer_step(self, epoch, batch_idx, optimizer, optimizer_idx, optimizer_closure, on_tpu, using_native_amp, using_lbfgs):
        ...
        if self.trainer.global_step < warmup_step:
            ...
            for pg in optimizer.param_groups:
                if 'backbone.' not in pg['layer_name'] and 'loftr_coarse.' not in pg['layer_name'] and \
                        'ladder.' not in pg['layer_name'] and 'up_block1.' not in pg['layer_name'] and 'loftr_coarse_4c.' not in pg['layer_name']:
